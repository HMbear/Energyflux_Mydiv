...
###Asing the environmental temperature based on the month and depth
###Here is the hourly temperature, attained from 21 days poriors to sampling date, they are differed by time, and depth 
  mutate(Tk = ifelse(month == "May" & depth == 5,Temp_AprMay_soil5cm,  
                     ifelse (month == "May" & depth == 510, Temp_AprMay_soil10cm, 
                             ifelse(month == "September" & depth == 5, Temp_AugSep_soil5cm, 
                                  Temp_AugSep_soil10cm)))) %>% 
